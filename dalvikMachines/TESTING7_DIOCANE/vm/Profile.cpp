...
static void getSample(Thread* thread)
{
  ALOGD("TRACE_DEBUG: INSIDE  getSample");
    ...
    if (oldStackTrace == NULL) {
	  ...
	  ALOGD("TRACE_DEBUG RETURN getSample. stack method no %d: %s", i, newStackTrace[i]->name);
...
char *objectToString(Thread *self, Object *object) {
  ALOGD("TRACE_DEBUG: INSIDE  objectToString");
    ...
    if (str == NULL) {
      ALOGD("TRACE_DEBUG: RETURN  objectToString: NULL");
      return NULL;
    }
...
void handle_method(Thread *self, const Method *method, MethodTraceState *state, int type, u4 *args) {
  ALOGD("TRACE_DEBUG: INSIDE  handle_method");
    ...
    ALOGD("TRACE_DEBUG: INSIDE handle_method. Printing values:");
...
void handle_return(Thread *self, const Method *method, MethodTraceState *state, JValue *retval) {
  ALOGD("TRACE_DEBUG: INSIDE  handle_return");
    ...
    ALOGD("TRACE_DEBUG handle_return Printing:");
#if ALOGD_TRACE_ENABLED
...
void handle_throws(Thread *self, const Method *method, MethodTraceState *state, JValue *retval, int action) {
  ALOGD("TRACE_DEBUG: INSIDE  handle_throws");
    ...
    ALOGD("TRACE_DEBUG: INSIDE handle_throws. Printing:");
    // ALOGD("%sthrows %s\n", whitespace, classDescriptor);
#if ALOGD_TRACE_ENABLED
    ALOGD("%sthrows %s\n", whitespace, classDescriptor);
#endif
...
/*      LOGD_TRACE("pDvmDex->filename: %s\n",caller_clazz->pDvmDex->fileName); */
    ...
    if (action == METHOD_TRACE_ENTER) {
      ...
      ALOGD("TRACE_DEBUG: dvmMethodTraceAdd. METHOD_TRACE_ENTER for method: %s", method->name);
      ...
      ALOGD("TRACE_DEBUG: dvmMethodTraceAdd. METHOD_TRACE_EXIT for method: %s", method->name);
        ...
        /* We are unrolling... */
      ALOGD("TRACE_DEBUG: dvmMethodTraceAdd. METHOD_TRACE_UNROLL for method: %s", method->name);
        self->depth = (self->depth == 0 ? 0 : self->depth-1);
