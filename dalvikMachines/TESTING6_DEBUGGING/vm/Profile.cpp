...
{
  ALOGD("TRACE_DEBUG: INSIDE  getStackTrace");
...
{
  ALOGD("TRACE_DEBUG: INSIDE  getSample");
    ...
    if (oldStackTrace == NULL) {
        ...
        for (int i = newLength - 1; i >= 0; --i) {
            dvmMethodTraceAdd(thread, newStackTrace[i], METHOD_TRACE_ENTER,
                              cpuClockDiff, wallClockDiff, TRACE_PROFILE, NULL);
        }
    } else {
        /*
...
{
  ALOGD("TRACE_DEBUG: INSIDE  runSamplingThread");
...
{
  ALOGD("TRACE_DEBUG: INSIDE  dvmProfilingStartup");
    /*
...
{
  ALOGD("TRACE_DEBUG: INSIDE  dvmProfilingShutdown");
#ifdef UPDATE_MAGIC_PAGE
...
{
  ALOGD("TRACE_DEBUG: INSIDE  updateActiveProfilers");
...
{
  ALOGD("TRACE_DEBUG: INSIDE  freeThreadStackTraceSamples");
    Thread* thread;
...
static void dumpThreadList(FILE* fp) {
  ALOGD("TRACE_DEBUG: INSIDE  dumpThreadList");
    dvmLockThreadList(NULL);
    for (Thread* thread = gDvm.threadList; thread != NULL; thread = thread->next) {
...
{
  ALOGD("TRACE_DEBUG: INSIDE  dumpMarkedMethods");
...
{
  ALOGD("TRACE_DEBUG: INSIDE  dumpMethodList");
...
{
  ALOGD("TRACE_DEBUG: INSIDE  dvmMethodTraceStart");
...
{
    ALOGD("TRACE_DEBUG: INSIDE  markTouchedMethods");
...
{
  ALOGD("TRACE_DEBUG: INSIDE  dvmGetMethodTracingMode");
...
{
  ALOGD("TRACE_DEBUG: INSIDE  dvmMethodTraceStop");
    MethodTraceState* state = &gDvm.methodTrace;
...
bool prep_log(Thread *self) {
  ALOGD("TRACE_DEBUG: INSIDE  prep_log");
...
char *convertDescriptor(const char *descriptor) {
    ALOGD("TRACE_DEBUG: INSIDE  convertDescriptor");
...
char *objectToString(Thread *self, Object *object) {
  ALOGD("TRACE_DEBUG: INSIDE  objectToString");
...
char *getThis(Thread *self, const Method *method, u4 *args) {
  ALOGD("TRACE_DEBUG: INSIDE  getThis");
...
char **getParameters(Thread *self, const Method *method, int parameterCount, u4 *args) {
  ALOGD("TRACE_DEBUG: INSIDE  getParameters");
...
char *getParameterString(Thread *self, const Method *method, char **parameters, int parameterCount) {
  ALOGD("TRACE_DEBUG: INSIDE  getParametersString");
    int i;
...
char *getWhitespace(int depth) {
  ALOGD("TRACE_DEBUG: INSIDE  getWhitespace");
...
void handle_method(Thread *self, const Method *method, MethodTraceState *state, int type, u4 *args) {
  ALOGD("TRACE_DEBUG: INSIDE  handle_method");
    int i;
...
void handle_return(Thread *self, const Method *method, MethodTraceState *state, JValue *retval) {
  ALOGD("TRACE_DEBUG: INSIDE  handle_return");
    char *whitespace = getWhitespace(self->depth);
...
void handle_throws(Thread *self, const Method *method, MethodTraceState *state, JValue *retval, int action) {
  ALOGD("TRACE_DEBUG: INSIDE  handle_throws");
    char *whitespace = getWhitespace(self->depth);
...
void dvmMethodTraceReadClocks(Thread* self, u4* cpuClockDiff,
                              u4* wallClockDiff)
{
  ALOGD("TRACE_DEBUG: INSIDE  dvmMethodTraceReadClocks");
#if defined(HAVE_POSIX_CLOCKS)
    if (useThreadCpuClock()) {
...
{
  ALOGD("TRACE_DEBUG: INSIDE  dvmFastMethodTraceExit");  
      if (self->interpBreak.ctl.subMode & kSubModeMethodTrace) {
        u4 cpuClockDiff = 0;
        u4 wallClockDiff = 0;
        dvmMethodTraceReadClocks(self, &cpuClockDiff, &wallClockDiff);
        dvmMethodTraceAdd(self, self->interpSave.method,
                          METHOD_TRACE_EXIT, cpuClockDiff, wallClockDiff, TRACE_PROFILE, NULL);
    }
}
...
{
  ALOGD("TRACE_DEBUG: INSIDE  dvmFastNativeMethodTraceExit");
      if (self->interpBreak.ctl.subMode & kSubModeMethodTrace) {
        u4 cpuClockDiff = 0;
        u4 wallClockDiff = 0;
        dvmMethodTraceReadClocks(self, &cpuClockDiff, &wallClockDiff);
...
{
  ALOGD("TRACE_DEBUG: INSIDE  dvmEmitEmulatorTrace");
#ifdef UPDATE_MAGIC_PAGE
    /*
